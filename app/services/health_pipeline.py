...
def ingest_apple_health(raw_records: Iterable[dict[str, Any]]) -> dict[str, Any]:
    ...
    The mobile client sends a neutral record contract. Cumulative values such as
    steps and sleep are aggregated on-device before upload; point-in-time metrics
    such as HRV, resting heart rate, weight and body composition preserve sample time.
    """
    init_db()
    ...
    with get_connection() as conn:
        for day, records in sorted(grouped.items()):
            ...
            body = {
                k: latest[k]["value"]
                for k in ("weight_kg", "bmi", "body_fat_percent", "muscle_mass_kg", "lean_body_mass_kg")
                if k in latest
            }
            activity = {
                k: latest[k]["value"]
            ...
            if body:
                ...
                conn.execute(
                    """INSERT INTO body_metrics(
                        record_date,weight_kg,bmi,body_fat_percent,muscle_mass_kg,lean_body_mass_kg,source,confidence
                    ) VALUES(?,?,?,?,?,?,?,?)""",
                    (
                        day,
                        body.get("weight_kg"),
                        body.get("bmi"),
                        body.get("body_fat_percent"),
                        body.get("muscle_mass_kg"),
                        body.get("lean_body_mass_kg"),
                        "Apple Health",
                        "E" if conflicts else "B",
                    ),
                )
            if activity:
                ...
                conn.execute(
                    """INSERT INTO activity_metrics(
                        record_date,steps,active_calories,exercise_minutes,distance_km,vo2max,resting_heart_rate,hrv_ms,source
                    ) VALUES(?,?,?,?,?,?,?,?,?)""",
                    (
                        day,
                        activity.get("steps"),
                        activity.get("active_calories"),
                        activity.get("exercise_minutes"),
                        activity.get("distance_km"),
                        activity.get("vo2max"),
                        activity.get("resting_heart_rate"),
                        activity.get("hrv_ms"),
                        "Apple Health",
                    ),
                )
            if sleep:
                ...
                conn.execute(
                    "INSERT INTO sleep_metrics(record_date,total_sleep_hours,source) VALUES(?,?,?)",
                    (day, sleep.get("total_sleep_hours"), "Apple Health"),
                )
...
def build_daily_snapshot(
    ...
    try:
        def one(table: str):
            row = conn.execute(
                f"SELECT * FROM {table} WHERE record_date=? ORDER BY id DESC LIMIT 1",
                (day,),
            ).fetchone()
        ...
        body, activity, sleep, nutrition = (
            one("body_metrics"),
            one("activity_metrics"),
            one("sleep_metrics"),
            one("nutrition_records"),
        )
        snapshot = {
            "record_date": day,
            "body": body,
        ...
        conn.execute(
            """INSERT INTO daily_snapshots(record_date,payload_json,quality_json,updated_at)
            ...
            (
                ...
                __import__("json").dumps(snapshot["quality"], ensure_ascii=False),
            ),
        )
        if owns_conn:
            conn.commit()
